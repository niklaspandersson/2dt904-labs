...
Install the neccessary python packages by using the requirements-file in the root of this repository.
...
1. Use the code in section 2.1 of the book as a reference. We don't need the abstractions that they're using for this lab, for now just focus on the minimal code to get the objective done. That is, to display a window using the `pygame` package.
1. If you're on a linux environment and encounter issues with creating the window and the opengl context try adding the following to your code:
   ```python
   import os
...
- Since we're not animating anything yet, we only need to draw our image once. That is, triggering the pipeline using `glDrawArrays`. I mention this because the book puts the draw call in a loop, which is not strictly neccessary for us at this moment. But it will be once we want to animate stuff.
  - But if you do encounter problems with getting your point to show up, it's worth trying to move the `glDrawArrays` and `pygame.display.flip()` to the event-loop.

## 4. Experiment on your own

These are some examples of things you might want continue experimenting with:

- Change the values that are assigned to `gl_Position` and `fragColor` in the respective shader to get an idea of what they do and the valid ranges.
...
- Move on and implement support for keyboard input (section 2.5 in the book) so that you can add simple interactivity. Like toggling animation, changing speed of animation and/or moving things around.
