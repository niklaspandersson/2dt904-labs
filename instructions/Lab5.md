...
1. Run `lab5-1-start.py` to see why lighting is often required when we do 3d graphics.
2. Examine `setupCube` to see that it provides both normals and colors that we can use.
...
7. Add an ambient component to lighten the parts of the model that is not facing towards the light source.

## 2. SPECULAR LIGHTING - Code along

1. Run `lab5-2-start.py` to "see" our new model.
2. Copy the code for diffuse lighting from the `lab5-1` shader program.
